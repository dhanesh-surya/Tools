...
const AppContent: React.FC = () => {
  ...
  // Tailwind Dark Mode - Toggle 'dark' class on html element
  useLayoutEffect(() => {
  ...
  return (
    <div className="min-h-screen flex flex-col font-sans bg-white dark:bg-slate-950 text-slate-900 dark:text-slate-100 transition-colors duration-300">
      ...
      {/* --- HEADER --- */}
      <header className="fixed top-0 left-0 right-0 z-50 h-16 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border-b border-slate-200/50 dark:border-slate-800/50 flex items-center justify-between px-4 lg:px-8 transition-all duration-500 shadow-soft dark:shadow-dark-soft">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            title={darkMode ? 'Dark Mode (Click to switch to Light)' : 'Light Mode (Click to switch to Dark)'}
            aria-label={darkMode ? 'Switch to light mode' : 'Switch to dark mode'}
          >
            {darkMode ? <Moon size={20} className="animate-pulse" /> : <Sun size={20} className="animate-spin-slow" />}
          </button>
      ...
      <div className="flex pt-16 flex-1">
        ...
        {/* --- SIDEBAR --- */}
        <aside className={`fixed lg:sticky top-16 left-0 h-[calc(100vh-4rem)] w-64 bg-white/90 dark:bg-slate-900/90 backdrop-blur-xl border-r border-slate-200/50 dark:border-slate-800/50 transform ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 transition-all duration-500 z-40 overflow-y-auto shadow-soft-lg dark:shadow-dark-lg`}>
          <div className="p-4">
            {/* Search */}
            ...
            <nav className="space-y-1">
              ...
              <button
                ...
                className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-semibold transition-all duration-200 group relative overflow-hidden hover:scale-105 hover:shadow-xl ${activeCategory === 'Favorites' && !activeToolId
              ...
              >
                ...
                <Star size={18} className={`transition-transform duration-200 ${activeCategory === 'Favorites' && !activeToolId ? 'scale-110 fill-current' : 'group-hover:scale-110'
                  }`} />
              ...
              {CATEGORIES.map(cat => (
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-semibold transition-all duration-200 group relative overflow-hidden hover:scale-105 hover:shadow-xl ${activeCategory === cat.id && !activeToolId
                ...
                >
                  ...
                  <cat.icon size={18} className={`transition-transform duration-200 ${activeCategory === cat.id && !activeToolId ? 'scale-110' : 'group-hover:scale-110'
                    }`} />
            ...
            <div className="mt-8">
              <div className="bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50 dark:from-slate-800 dark:via-slate-800 dark:to-slate-800 rounded-xl p-5 text-center border border-indigo-200 dark:border-slate-700 shadow-lg">
                  ...
                  {filteredTools.length === 0 && searchQuery !== '' ? (
                    ...
                    filteredTools.map((tool, index) => (
                      <div
                        ...
                        className="group bg-white/90 dark:bg-slate-900/90 backdrop-blur-sm rounded-2xl p-6 border border-slate-200/80 dark:border-slate-800/80 shadow-soft hover:shadow-soft-xl dark:shadow-dark-soft dark:hover:shadow-dark-lg hover:-translate-y-3 hover:scale-[1.02] hover:border-indigo-400/50 dark:hover:border-indigo-500/50 transition-all duration-500 cursor-pointer relative overflow-hidden animate-fade-in-up"
                      >
                        {/* Favorite indicator */}
                        ...
                        <div className="flex justify-between items-start mb-4 relative z-10">
                          <div className={`p-3.5 rounded-2xl transition-all duration-300 group-hover:scale-110 group-hover:rotate-6 shadow-lg group-hover:shadow-xl ${tool.category === 'Finance' ? 'bg-gradient-to-br from-emerald-400 to-green-600 text-white' :
                          ...
                          <div className="flex items-center gap-2">
                            <button
                              ...
                              className={`p-2 rounded-xl transition-all duration-200 hover:scale-110 shadow-md ${favorites.has(tool.id)
      ...
      {toast && (
        <div className="fixed bottom-6 right-6 z-50 animate-fade-in">
          <div className={`flex items-center gap-3 px-4 py-3 rounded-xl shadow-lg backdrop-blur-xl border ${toast.type === 'success'
